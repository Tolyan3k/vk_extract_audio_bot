...
def process_converted_vk_video(video_content_id: str) -> Optional[str]:
...
:
    response = __init__.vk_audio_api.method('video.get',
    ...
    try:
        ...
        video_file_name = VideoService.download_video(
            player_link, 
            VideoDownloadSettings(VIDEO_MIN_DURATION, VIDEO_MAX_DURATION, None, f"{video_file_name}"),
            video_info,
        )

        if ConverterService.has_audio(video_file_name):
            audio_convert_settings = AudioConvertSettings(AUDIO_MIN_DURATION)
            audio_file_name = ConverterService.convert_video_to_audio(video_file_name, audio_convert_settings)
    ...
    try:
        archive_group_content_id = VkAudioService.add_audio(audio_content_id, VK_ARCHIVE_GROUP_ID)
        if archive_group_content_id:
            archive_group_content_id = '-' + VK_ARCHIVE_GROUP_ID + '_' + str(archive_group_content_id)
    except:
        return None
    finally:
        try:
...
def get_author_name(video_info):
    ...
    try:
        if video_info.author < 0:
            group_objs = session_api.groups.getById(group_ids= f'{-video_info.author}')
...
def send_audios_to_user(msg: dict, videos: list[dict], vk_audio_content_ids: list[str]):
    ...
    parts = (len(vk_audio_content_ids) + (VK_MAX_ATTACHMENTS - 1)) // VK_MAX_ATTACHMENTS
    for part in range(0, parts):
        message =  f'[{part + 1}/{parts}]' if parts >= 2 else error_msg_text
        audio_start_index = VK_MAX_ATTACHMENTS * part
        audio_end_index = min(VK_MAX_ATTACHMENTS * (part + 1), len(vk_audio_content_ids))

        send_replied_msg_else_not(__init__.vk_main_group_api_session, 
            message= message,
            reply_to= msg['id'],
            attachment= "audio" \
                + ",audio".join(vk_audio_content_ids[audio_start_index : audio_end_index]),
            user_id= msg['peer_id'],
            random_id= random.randint(0, 1 << 31),
        )

    if error_msg_text \
    and (len(vk_audio_content_ids) > VK_MAX_ATTACHMENTS or parts == 0):
        send_replied_msg_else_not(__init__.vk_main_group_api_session, 
...
def need_process_msg(msg):
    if msg is None:
        return False
    elif msg['from_id'] == -int(VK_MAIN_GROUP_ID):
        return False
...
def process_msg(msg):
    ...
    for video_id in range(0, len(videos)):
        try:
            ...
            logger.info(f'Во время обработки видео {videos[video_id]} произошла ошибка.')
            logger.info(error)
...
def edit_vk_audio_if_video_public_or_non_vk(
    ...
    if new_video and (is_video_public or not is_platform_vk):
        VkAudioService.edit_audio(archive_group_content_id, 
...
def process_msg_new(msg_id):
    ...
    try:
        ...
        logger.info(f"Не удалось обработать сообщение с id = {msg_id}. Перехожу к следующему сообщению.")
...
def process_unanswered_messages():
    logger.info('Начинаю проверять неотвеченные сообщения.' \
    ...
    logger.info('Закончил проверять неотвеченные сообщения.' \
...
def send_msg_if_queued(msg_id, msg_peer_id = None):
    ...
    count += 1 if processing_queue.qsize() else 0 # 1 if not processing_queue.qsize() else 0 
...
def process_longpoll():
    ...
    for event in __init__.vk_bot_longpoll.listen():
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            sleep(1) # Чтобы вложение успело прогрузиться, если пользователь пришлёт только ссылку
